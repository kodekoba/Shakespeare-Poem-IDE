export function destructuring(){
  const feast = {    //the object we will destructure
    appetizer: 'Spring Rolls',
    entree: 'Enchiladas',
    dessert: 'Apple Pie',
    beverages: {
      wine: 'Merlot',
      water: 'Sparkling',
    }
  }
  
  let {         //to destructure an object we declare a
    appetizer,  //new variable and then choose any number
    dessert     //of keys to create into stand alone variables
  } = feast    //we also specify the object we're destructuring
  
  console.log('appetizer: ', appetizer)
  console.log('dessert: ', dessert)
  
  
  let {entree, beverages: {wine}} = feast
  let {water} = feast.beverages
  let {dessert: favorite} = feast
  
  console.log(entree)         //"Enchiladas"
  console.log(wine)           //"Merlot"
  console.log(water)          //"Sparkling"
  console.log(favorite)       //"Apple Pie"
  
  const horsdoeuvre = ['crab cakes', 'spring rolls', 'fried pickles', 'caviar']
  
  let [, , southernClassico, grossFishEggs] = horsdoeuvre // skip over per comma
  // replaces the use of something like console.log(horsdoeuvre[0])
  console.log(grossFishEggs)    //"caviar"
  console.log(southernClassico) //"fried pickles"
}
